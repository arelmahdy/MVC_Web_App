...
namespace MVC_Web_App.Controllers
{
    ...
    public class HomeController : Controller
    {
        ...
        {
            var post = db.Posts.OrderByDescending(x => x.Id).Include(x => x.SubCategory);
            cartCount = CartCount(); 
        ...
        public async Task<IActionResult> PostView(int? id)
        {
            ...
            var post = await db.Posts.Include(x => x.SubCategory).FirstOrDefaultAsync(x => x.Id == id);
            if (post == null)
            {
                return NotFound();
            }
            ...
            string cardtype, string cardname, long cardnumber, DateTime expire, int cvv, int cartId, string userId)
        {
            ...
            {
                if (cardtype == null || cardname == null || cardnumber <= 0 || cvv <= 0)
                {
                    TempData["fail"] = "يجب ملئ جميع البيانات بمربعات النص";
                    return RedirectToAction("Cart", new { id = userId });
                }
